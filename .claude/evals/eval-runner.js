...
// Claude CLI 경로
const CLAUDE_CLI =
  process.env.CLAUDE_CLI || "/Users/younghwankang/.local/bin/claude";
...
function parseSimpleYaml(content) {
  ...
  for (let line of lines) {
    ...
    if (trimmed.startsWith("desc:")) {
      ...
      currentChecks.push({
        output_contains: trimmed
          .replace("- output_contains:", "")
          .trim()
          .replace(/"/g, ""),
      });
      inOutputContainsAny = false;
      ...
      if (lastCheck.output_contains_any) {
        lastCheck.output_contains_any.push(
      ...
      if (lastCheck.contains) {
        lastCheck.contains.pattern = trimmed
          .replace("pattern:", "")
          .trim()
          .replace(/"/g, "");
      } else if (lastCheck.not_contains) {
        lastCheck.not_contains.pattern = trimmed
          .replace("pattern:", "")
          .trim()
          .replace(/"/g, "");
      } else if (lastCheck.contains_any) {
        lastCheck.contains_any.pattern = trimmed
          .replace("pattern:", "")
          .trim()
          .replace(/"/g, "");
      }
      ...
      const lastCheck = currentChecks[currentChecks.length - 1];
      if (lastCheck.contains) {
        lastCheck.contains.search = trimmed
          .replace("search:", "")
          .trim()
          .replace(/"/g, "");
      } else if (lastCheck.not_contains) {
        lastCheck.not_contains.search = trimmed
          .replace("search:", "")
          .trim()
          .replace(/"/g, "");
      }
      ...
      const lastCheck = currentChecks[currentChecks.length - 1];
      if (lastCheck.contains_any) {
        lastCheck.contains_any.searches.push(
          trimmed.replace(/^- "/, "").replace(/"$/, ""),
        );
      }
    } else if (
      ...
      currentGrader.threshold = parseInt(
...
class EvalRunner {
    ...
    for (const file of graderFiles) {
      ...
      if (fs.existsSync(graderPath)) {
        ...
        try {
          ...
          console.warn(
            `[EvalRunner] Grader load warning: ${file} - ${e.message}`,
          );
        }
      }
    }
  ...
  runClaudeAgent(prompt, workDir, timeout) {
    ...
    try {
      ...
      if (this.options.verbose) {
        console.log(`    [Claude] Exit code: ${result.exitCode}`);
        console.log(
          `    [Claude] stdout (${result.stdout.length} chars): ${result.stdout.substring(0, 200)}`,
        );
        if (result.stderr) {
          console.log(
            `    [Claude] stderr: ${result.stderr.substring(0, 200)}`,
          );
        }
      }
      ...
      if (
        allOutput.includes("credit balance is too low") ||
        allOutput.includes("rate limit")
      ) {
        result.error = "CREDIT_EXHAUSTED";
  ...
  async runTask(task) {
    ...
    try {
      ...
      const outcome = {
        ...
        creditExhausted: agentResult.creditExhausted || false,
        transcript,
        workDir,
      };
  ...
  async runTrials(task, k) {
    ...
    for (let i = 0; i < k; i++) {
      ...
      if (result.creditExhausted) {
        console.error("\n❌ Credit balance is too low. Aborting eval.");
        console.error("   Wait for session reset or check your plan limits.");
        process.exit(1);
      }
  ...
  async gradeResult(task, outcome) {
    ...
    if (this.options.dryRun && outcome.transcript?.simulated) {
      const simSuccess =
        outcome.transcript?.output?.includes("BUILD SUCCESS") ||
        outcome.transcript?.turns?.some((t) =>
          t.content?.includes("successfully"),
        );
    ...
    if (
      this.graders.state &&
      task.graders?.some((g) => g.type === "state_check")
    ) {
      const stateConfig = task.graders.find((g) => g.type === "state_check");
      const origBasePath = this.graders.state.basePath;
      this.graders.state.basePath =
        task.category === "build" ? PROJECT_ROOT : workDir;
      try {
    ...
    const firstPassPassed =
      grades.deterministic?.passed !== false && grades.state?.passed !== false;
    if (
      firstPassPassed &&
      this.graders.llm &&
      task.graders?.some((g) => g.type === "llm_rubric")
    ) {
    ...
    if (this.options.verbose) {
      console.log(
        `    [Grade] deterministic: ${grades.deterministic?.passed ?? "N/A"}, state: ${grades.state?.passed ?? "N/A"}, llm: ${grades.llm?.passed ?? "N/A"}, overall: ${grades.overall}`,
      );
      if (grades.deterministic?.checks) {
        for (const check of grades.deterministic.checks) {
          console.log(
            `      ${check.passed ? "✓" : "✗"} ${check.type}: ${check.reason}`,
          );
        }
      }
    }
  ...
  async runSuite(suiteName) {
    ...
    console.log(
      `   Mode: ${this.options.dryRun ? "DRY RUN" : "LIVE (Claude CLI)"}`,
    );
    ...
    for (const task of suite.tasks) {
      ...
      const taskResult = {
        ...
        avgDuration:
          trials.reduce((sum, t) => sum + (t.duration || 0), 0) / trials.length,
      ...
      // 결과 출력
      const effectivePassed = taskResult.expectFailure
        ? taskResult.actualPassed
        : taskResult.successes > 0;
      const status = effectivePassed ? "✅" : "❌";
      console.log(
        `  ${status} Pass@1: ${(taskResult.passAt1 * 100).toFixed(1)}%`,
      );
      console.log(`     Pass@3: ${(taskResult.passAt3 * 100).toFixed(1)}%`);
      console.log(`     Pass^3: ${(taskResult.passExp3 * 100).toFixed(1)}%`);
      if (!this.options.dryRun) {
        console.log(
          `     Avg Duration: ${(taskResult.avgDuration / 1000).toFixed(1)}s`,
        );
      }
    }
    ...
    suiteResults.summary.failed =
      suiteResults.summary.total - suiteResults.summary.passed;
    suiteResults.summary.passAt1 =
      suiteResults.tasks.reduce((sum, t) => sum + t.passAt1, 0) /
      suiteResults.tasks.length;
    suiteResults.summary.passAt3 =
      suiteResults.tasks.reduce((sum, t) => sum + t.passAt3, 0) /
      suiteResults.tasks.length;
    suiteResults.summary.passExp3 =
      suiteResults.tasks.reduce((sum, t) => sum + t.passExp3, 0) /
      suiteResults.tasks.length;
  ...
  printSummary() {
    ...
    for (const result of this.results) {
      ...
      console.log(
        `  Avg Pass@1: ${(result.summary.passAt1 * 100).toFixed(1)}%`,
      );
      console.log(
        `  Avg Pass@3: ${(result.summary.passAt3 * 100).toFixed(1)}%`,
      );
      console.log(
        `  Avg Pass^3: ${(result.summary.passExp3 * 100).toFixed(1)}%`,
      );
