...
/**
 * 에이전트 가용성 체크
 * 현재 활성 에이전트 수를 확인하고 초과 시 경고
 *
 * @returns {Object} { available: boolean, activeCount: number, maxConcurrent: number, warning?: string }
 */
function checkAgentAvailability() {
  try {
    const parallelStatePath = path.join(__dirname, '../ace-framework/parallel-state.json');
    if (!fs.existsSync(parallelStatePath)) {
      return { available: true, activeCount: 0, maxConcurrent: CONFIG.maxConcurrentAgents };
    }
    const state = JSON.parse(fs.readFileSync(parallelStatePath, 'utf8'));
    const activeCount = (state.activeAgents || []).length;
    const available = activeCount < CONFIG.maxConcurrentAgents;
    const result = { available, activeCount, maxConcurrent: CONFIG.maxConcurrentAgents };
    if (!available) {
      result.warning = 'Max concurrent agents (' + CONFIG.maxConcurrentAgents + ') reached. Wait for active agents to complete.';
    }
    return result;
  } catch (e) {
    return { available: true, activeCount: 0, maxConcurrent: CONFIG.maxConcurrentAgents };
...
/**
 * 성능 기반 에이전트 추천 (feedback-loop 연동)
 *
 * feedback-loop의 실행 메트릭을 분석하여 작업 유형에 가장 적합한 에이전트를 추천합니다.
 *
 * @param {string} taskType 작업 유형 (예: 'service_build', 'feature_development')
 * @returns {Object} { recommended: string, performance: object, alternatives: string[] }
 */
function recommendAgent(taskType) {
  const FEEDBACK_LOOP_PATH = path.join(__dirname, 'feedback-loop.js');

  // 1. capability matching 기본 추천
  const agentModel = loadAgentModel();
  let baseRecommendation = 'kiips-developer';
  if (agentModel && agentModel.capability_matching && agentModel.capability_matching.task_types) {
    const taskConfig = agentModel.capability_matching.task_types[taskType];
    if (taskConfig && taskConfig.primary_agent) {
      baseRecommendation = taskConfig.primary_agent;
    }
  }

  // 2. feedback-loop에서 성능 데이터 로드
  let performance = null;
  try {
    if (fs.existsSync(FEEDBACK_LOOP_PATH)) {
      const feedbackLoop = require(FEEDBACK_LOOP_PATH);
      const report = feedbackLoop.generateSummaryReport({ days: 7 });

      if (report && report.summary && report.summary.totalExecutions > 0) {
        // 에이전트별 성공률 및 효율성 분석
        const dateKeys = Object.keys(report.layerPerformance || {});
        const agentStats = {};

        // aggregated 데이터에서 에이전트별 통계 추출
        const metricsPath = path.join(__dirname, '../ace-framework/telemetry/execution-metrics.json');
        if (fs.existsSync(metricsPath)) {
          const metricsData = JSON.parse(fs.readFileSync(metricsPath, 'utf8'));
          const recentMetrics = (metricsData.metrics || []).filter(function(m) {
            return m.taskType === taskType;
          });

          for (var i = 0; i < recentMetrics.length; i++) {
            var m = recentMetrics[i];
            var agentId = m.agentId || 'unknown';
            if (!agentStats[agentId]) {
              agentStats[agentId] = { count: 0, successCount: 0, totalEfficiency: 0 };
            }
            agentStats[agentId].count++;
            if (m.success) agentStats[agentId].successCount++;
            agentStats[agentId].totalEfficiency += (m.efficiency || 0);
          }
        }

        // 성능 점수 계산
        var bestAgent = baseRecommendation;
        var bestScore = 0;
        var agentNames = Object.keys(agentStats);

        for (var j = 0; j < agentNames.length; j++) {
          var agent = agentNames[j];
          var stats = agentStats[agent];
          if (stats.count < 2) continue; // 최소 2회 실행 필요

          var successRate = stats.successCount / stats.count;
          var avgEfficiency = stats.totalEfficiency / stats.count;
          var score = (successRate * 0.6) + (avgEfficiency * 0.4); // 가중 점수
        ...
        performance = {
          basedOnExecutions: report.summary.totalExecutions,
          successRate: report.summary.successRate + '%',
          avgEfficiency: report.summary.avgEfficiency + '%',
          agentStats: agentStats,
          performanceAdjusted: bestAgent !== baseRecommendation
        };

        if (bestAgent !== baseRecommendation && bestScore > 0) {
          baseRecommendation = bestAgent;
        }
      }
    }
  } catch (e) {
    // fail-open: 성능 데이터 없으면 기본 추천 유지
  }

  // 3. Manager 라우팅 확인
  var manager = routeToManager(taskType);

  // 4. 대안 에이전트 목록
  var alternatives = [];
  var allAgents = ['kiips-developer', 'kiips-architect', 'kiips-ui-designer',
    'kiips-realgrid-generator', 'code-simplifier', 'checklist-generator'];
  for (var k = 0; k < allAgents.length; k++) {
    if (allAgents[k] !== baseRecommendation) {
      alternatives.push(allAgents[k]);
    }
  }
