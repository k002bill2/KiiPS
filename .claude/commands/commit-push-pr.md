...
cd KiiPS-HUB
# [빌드담당자] mvn clean package -pl :{{service}} -am
...
cd KiiPS-HUB
# [빌드담당자] mvn test -pl :{{service}} -DskipTests=false
...
cd KiiPS-HUB
# [빌드담당자] mvn clean package test -pl :{{service}} -am -DskipTests=false -T 4
```

### Incremental Build
```bash
# Only rebuild changed modules (Maven's -pl flag already does this)
# [빌드담당자] mvn package -pl :{{service}} -am
```
...
```bash
# 1. Build (manual)
# [빌드담당자] cd KiiPS-HUB && mvn clean package -pl :KiiPS-FD -am
# ⏱️ 3-4 minutes

# 2. Test (often skipped)
# [빌드담당자] mvn test -pl :KiiPS-FD -DskipTests=false
...
```bash
cd KiiPS-HUB
# [빌드담당자] mvn clean install -U  # Force update dependencies
# Retry workflow
```
