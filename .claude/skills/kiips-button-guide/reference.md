...
> 📌 **정본 43행 전체 정의 → [button-order.md](./button-order.md)**
> (출처: 팀 Google Sheets "버튼 정렬 순서" 시트. 순번 오름차순으로 배치하고, 화면에 없는 순번은 건너뛴다)
...
| 14 | 전송 | `icon_send` |
| 16 | 저장 | `icon_save` |
| 31/33 | 행추가/행삭제 | `icon_addRow` / `icon_delRow` |
| 39 | 삭제 | `icon_trash` |
| 40 | 인쇄 | `icon_print` |
| 42 | 엑셀 | `icon_excel` |
| 43 | 도움말 | `icon_question` |

```
[어드민] → [조회] → [등록] → [결재상신] → [전송] → [저장] → [삭제] → [인쇄] → [엑셀] → [도움말]
```

⚠️ **엑셀(42)은 항상 도움말(43) 바로 앞**이다. 엑셀만 `btn-primary`(채움 드롭다운)로 렌더되므로
순서가 틀리면 채워진 파란 버튼이 줄 가운데 박혀 배열이 어긋나 보인다.
⚠️ 시트의 `class name` 열은 5행에 오타가 있다 — 반드시 [button-order.md](./button-order.md) 의 교정본을 쓸 것.
