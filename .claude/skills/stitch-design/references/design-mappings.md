...
## KiiPS-Specific Mappings

| KiiPS Term | Design Description |
|:---|:---|
| "검색필터" | "Horizontal form with labeled inputs, date pickers, and search/reset buttons in a collapsible panel" |
| "그리드" | "Data table with sortable headers, row selection, pagination, and Excel export" |
| "등록모달" | "Modal dialog with form fields, validation, and save/cancel buttons" |
| "버튼 툴바" | "Right-aligned toolbar with icon buttons: reload, add, delete, Excel, print" |
| "탭 패널" | "Horizontal tab navigation with content panels below" |
