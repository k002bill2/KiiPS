...
#   7) 외부 도구 진단 (누락은 ❌ 비치명적)
#   8) 플러그인 마켓플레이스/본체 자동 설치 (claude CLI, enabledPlugins==true 만, 멱등)
#   9) self-verify (실패 시 exit 1)
#
...
echo "▶ [1/9] 메모리 $(ls "$MEM_DST"/*.md 2>/dev/null | wc -l | tr -d ' ')개 → $MEM_DST"

# ── 2. 글로벌 사람작성 자산 ─────────────────────────────────────────────────
for d in commands agents output-styles; do
  [ -d "$SEED/global/$d" ] && rsync -a "$SEED/global/$d/" "$GLOBAL_DIR/$d/"
done
echo "▶ [2/9] 글로벌 commands/agents/output-styles 설치"
...
if [ -f "$SEED/global-settings.overlay.json" ]; then
  ...
  echo "▶ [3/9] 글로벌 settings overlay 병합 (5키, 기존 설정 보존)"
fi
...
if [ -f "$SEED/global/awesome-statusline.sh" ]; then
  ...
  echo "▶ [4/9] statusLine 설치 + 경로 $HOME 보정"
fi
  ...
  if command -v brew >/dev/null 2>&1; then
    ...
    if [ -n "$CUR" ] && [ "$CUR" != "$B/bin/jdtls" ]; then
      ...
      jq --arg b "$B" --arg oj "$OJ" \
        ...
        "$PS" > "$t" && mv "$t" "$PS"
      echo "▶ [5/9] jdtls/openjdk 경로를 brew prefix($B) 기준으로 보정"
    else
      echo "▶ [5/9] jdtls 경로 이미 정상 (Apple Silicon no-op)"
    fi
  else
    echo "▶ [5/9] ⚠ Homebrew 미설치 — Java LSP 경로 보정 생략. brew 설치 후 재실행 권장."
  fi
...
if [ -f "$LS" ] && command -v jq >/dev/null 2>&1; then
  ...
  echo "▶ [6/9] settings.local.json serena dead-entry 제거"
fi
...
# ── 8. 플러그인 마켓플레이스/본체 자동 설치 (claude CLI — 비대화/멱등 실측 검증) ──
# enabledPlugins 선언만으로는 본체가 안 받아짐(공식 docs: add→install 2단계). overlay 를
# 단일 진실원으로 jq 파싱 → marketplace add + plugin install. 네트워크 실패는 비치명적.
OVL="$SEED/global-settings.overlay.json"
if command -v claude >/dev/null 2>&1 && [ -f "$OVL" ]; then
  echo "▶ [8/9] 플러그인 자동 설치 (claude CLI)"
  # 8a) 마켓플레이스 등록: extraKnownMarketplaces 의 source (github=repo, git=url)
  while IFS= read -r src; do
    [ -z "$src" ] && continue
    if claude plugin marketplace add "$src" </dev/null >/dev/null 2>&1; then
      echo "    ✅ marketplace + $src"
    else
      echo "    ⚠ marketplace $src 등록 실패 (네트워크/권한 확인)"
    fi
  done < <(jq -r '(.extraKnownMarketplaces // {}) | to_entries[] | .value.source
                  | if .source=="git" then .url elif .source=="github" then .repo else empty end' "$OVL")
  # 8b) 본체 설치: enabledPlugins==true 만 (false 는 건너뜀)
  while IFS= read -r pl; do
    [ -z "$pl" ] && continue
    if claude plugin install "$pl" </dev/null >/dev/null 2>&1; then
      echo "    ✅ plugin + $pl"
    else
      echo "    ⚠ plugin $pl 설치 실패"
    fi
  done < <(jq -r '(.enabledPlugins // {}) | to_entries[] | select(.value==true) | .key' "$OVL")
else
  echo "▶ [8/9] ⚠ claude CLI 미발견 — 플러그인 자동설치 생략. Claude 설치 후 setup.sh 재실행 시 자동 설치됨."
fi

# ── 9. self-verify (증거 기반 완료 — seed 실제 개수 대비, 부분복사 탐지) ─────
echo "▶ [9/9] self-verify"
ERR=0
...
echo "  1) 플러그인 확인: claude plugin list  (위 [8/9]에서 자동설치됨 — claude CLI 부재 시에만 /plugin 수동)"
