...
작업 계획 수립을 위한 코드 탐색 및 영향 분석 에이전트 (읽기 전용).

> **범위 (2026-06-15 정리)** — 범용 계획/탐색은 빌트인 `Plan`·`Explore` 에이전트가 수행한다.
> 이 에이전트의 고유 가치는 아래 **KiiPS 의존성 체인·위험 매트릭스** 도메인 지식이다.
> spawn 권한(`Agent` 도구)은 본문이 구현·위임을 금지하므로 vestigial이라 제거했다.
