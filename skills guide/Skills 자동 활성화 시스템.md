...
```json
{
  "hooks": {
    "UserPromptSubmit": [
      {
        "type": "prompt",
        "prompt": "KiiPS 빌드 작업인가요? 그렇다면 kiips-maven-builder Skill을 사용하세요. KiiPS-HUB에서 빌드해야 합니다!"
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "echo '⚠️  pom.xml 변경됨 - KiiPS-HUB에서 재빌드 필요'"
        }]
      }
    ]
  }
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "sleep 5 && curl -s http://localhost:$PORT/actuator/health | jq"
        }]
      }
    ]
  }
...
*마지막 업데이트: 2025-12-28, KiiPS Hook 시스템 적용*

#skills-activation #hooks #automation #claude-code #kiips