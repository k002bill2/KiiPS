...
### KiiPS 대규모 작업 예시
- **작업명**: kiips-api-gateway-refactoring
- **파일**: `dev/active/kiips-api-gateway-refactoring/`
- **특징**: 20+ 마이크로서비스 통합, JWT 인증, 라우팅 규칙
- **복잡도**: 다중 서비스 의존성, 환경별 설정 분리

### 실무 활용
1. **Plan 문서**: Maven 빌드 순서, 서비스 의존성 그래프
2. **Context 문서**: API Gateway 설정, Spring Cloud Config
3. **Tasks 문서**: 서비스별 빌드/배포 체크리스트
...
- [ ] SVN 커밋 및 태그
- [ ] API Gateway 라우팅 테스트
- [ ] 각 마이크로서비스 Health Check

---

*"Documentation is Claude's memory, and memory is everything"*
*마지막 업데이트: 2025-12-28, KiiPS 프로젝트 적용*

#dev-docs #context-management #planning #kiips #enterprise