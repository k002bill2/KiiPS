...
While KiiPS uses `start.sh/stop.sh` scripts, PM2 concepts apply:

#### KiiPS 서비스 관리 패턴
```bash
# Service lifecycle (KiiPS native)
./start.sh    # 서비스 시작
./stop.sh     # 서비스 중지
./restart.sh  # 재시작 (stop + start)

# Log monitoring (PM2 equivalent)
tail -f logs/log.$(date "+%Y-%m-%d")-0.log

# Multiple services (similar to PM2 ecosystem)
for service in KiiPS-FD KiiPS-IL KiiPS-PG; do
  cd ../$service && ./start.sh &
done
```
...
2. **Log Analysis** (kiips-log-analyzer Skill)
   ```bash
   grep -i "error" logs/log.$(date "+%Y-%m-%d")-0.log
   ```

3. **API Gateway 테스트** (kiips-api-tester Skill)
   ```bash
...
| 기능 | PM2 | KiiPS |
|------|-----|-------|
| 프로세스 관리 | `pm2 start/stop` | `./start.sh/stop.sh` |
| 로그 | `pm2 logs` | `tail -f logs/*.log` |
| 상태 확인 | `pm2 status` | `curl actuator/health` |
| 환경변수 | `.env` | `app-*.properties` |

---

*"Make debugging bearable with PM2 (or your own service management)"*
*마지막 업데이트: 2025-12-28, KiiPS 서비스 관리 패턴 추가*

#pm2 #microservices #debugging #backend #kiips #spring-boot