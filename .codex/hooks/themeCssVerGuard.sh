...
# theme.css 의존성 SCSS 편집 후 header.jsp:85의 ?ver=YYMMDD_N 캐시 버스터 갱신 누락 감지
...
# header.jsp:85에서 theme.css?ver= 값 추출
CURRENT_VER=$(grep -oE 'theme\.css\?ver=[0-9]{6}_[0-9]+' "$HEADER_JSP" | head -1 | sed 's/theme\.css?ver=//')
[[ -z "$CURRENT_VER" ]] && exit 0

CURRENT_DATE=$(echo "$CURRENT_VER" | cut -d_ -f1)
TODAY=$(date +%y%m%d)
...
⚠️  theme.css 캐시 버스터 갱신 누락 감지
   ...
   같은 날 N번째 수정이면 _N 을 증가시키세요 (예: ${TODAY}_1, ${TODAY}_2 ...).
