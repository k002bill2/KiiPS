---
name: "source-command-periodic-cleanup"
description: "Periodic garbage collection - scan and report code hygiene issues"
---

# source-command-periodic-cleanup

Use this skill when the user asks to run the migrated source command `periodic-cleanup`.
...
grep -l 'confidence:.*0\.[0-2]' .Codex/learning/instincts/personal/*.md
```

#### 3-2. 장기 미사용 Instinct
- `updated` 필드가 30일 이상 이전인 instinct 검출
- 후보: 아카이빙 (.Codex/learning/instincts/archived/)
...
- Instinct 아카이빙/수명 관리만 필요하면 `/instinct-gc` 사용 — 본 커맨드는 코드 위생 스캔이 중심이며 instinct는 부가 모드일 뿐.
- 실제 코드 수정/리팩토링은 대상 아님 — 스캔·보고 전용.
