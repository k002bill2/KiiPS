...
```bash
cat .Codex/evals/eval-config.json | jq '.suites | keys'
```

선택한 스위트로 평가를 실행합니다:

```bash
node .Codex/evals/eval-runner.js --suite $SUITE --trials $TRIALS $FLAGS
```
...
- `.Codex/evals/results/latest.json` - 최근 결과
- `.Codex/evals/results/history.jsonl` - 히스토리
...
💾 Results saved to: .Codex/evals/results/latest.json
```
